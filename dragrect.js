...
/**
 *
 *  REFACTOR THIS LATER. Just testing out drawing in two table rows.
 *
 */
function createSvg2(d) {
    /**
     * drag behaviours
     */
    function hasCollidedWithObstacle(oldx, currentDragXPos) {
        var stopDrag = false;
        var isCursorLeftOfObstable = oldx <= data[1].obsXStart;
        var isCursorRightOfObstable = oldx >= data[1].obsXEnd;

        if(isCursorLeftOfObstable && currentDragXPos >= data[1].obsXStart) {
            return stopDrag = true;
        }

        if(isCursorRightOfObstable && currentDragXPos <= data[1].obsXEnd) {
            return stopDrag = true;
        }
        return stopDrag;
    }
    ...
    var tooltipDiv = d3.select("body").append("div")
        .attr("id", "brltooltip")
        .attr("class", "tooltip")
        .style("left", function(d) { return (data[1].brXStart) + "px";})
        .style("opacity", 0);

    var ltooltip = d3.select("body").append("div")
        .attr("id", "ltooltip")
        .attr("class", "tooltip")
        .style("left", function(d) { return (data[1].xStart) + "px";})
        .style("opacity", 0);
    ...
    var staticg = svg.append("g")
          .data([{x: data[1].obsXStart, y: data[1].obsYStart}]);

    // bar underneadth the back rectangle
    var brunderbar = brg.append("rect")    
        .attr("id", "brunderbar")
        .attr("x", function(d) { return d.x; })
        .attr("y", function(d) { return d.y + data[1].obsYEnd - data[1].obsYStart  + yOffset; })
        .attr("height", 5)
        .attr("width", brwidth)
        .attr("fill", "url(#diagonalPatterns)");
    ...
    var obsrect = staticg.append("rect")
        ...
        .attr("x", function(d) { return d.x; })
        .attr("y", function(d) { return d.y ; })
        .attr("height", data[1].obsYEnd - data[1].obsYStart)
        .attr("width", data[1].obsXEnd - data[1].obsXStart)
        .attr("fill", "red")
        .attr("fill-opacity", .9)
    ...
    var brdragbarright = brg.append("rect")
          .attr("x", function(d) { return d.x + brwidth - (brdragbarw / 2); })
          .attr("y", function(d) { return d.y + (dragbarw/2)  + yOffset; })
          .attr("id", "brdragright")
          .attr("height", brheight - brdragbarw)
          .attr("width", brdragbarw)
          .attr("fill", "blue")
